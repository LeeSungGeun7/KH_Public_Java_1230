...
// 최종 연산시 모든 요소를 소모한 해당 스트림은 더 이상 사용 할수 없습니다.
// 요소의 출력 : forEach()
// 요소의 소모 : reduce() => 첫번째와 두번째 요소를 가지고 연산을 수행한 뒤,
//                         그 결과와 세번째 요소를 가지고 또 다시 수행
// Optional : NPE(Null Pointer Exception)을 방지 할 수 있도록 도와주는 클래스입니다.
public class StreamEx7 {
    public static void main(String[] args) {
        ...
        OptionalInt rst1 = stream3.sorted().findFirst(); //정렬이후 첫번째 값을 가져옴
        System.out.println(rst1.getAsInt());

        OptionalInt rst2 = stream4.sorted().findAny(); // 정렬 이후 첫번째 값을
                                                       // 가져옴(벙렬 처리시에는 결과가 달라질수있음)
        ...
        System.out.println(stream6.allMatch(n->n>9)); // false
        System.out.println(stream7.noneMatch(n->n>100)); // false
        // 요소의 통계
