...
public class IpadPro {
    ...
    int network ; // 네트워크 : wi-fi , wi-fi+celluler
    String name ; // 각인 서비스 신청 시 이름 저장
    String productDate; //제품 생산일자
    String serialNum; // 제품의 일련번호 : iPad13128C2301111
    static int cnt = 0 ; // 클래스변수 (객체생성시 생성되지않고 클래스에 소속된 변수)

    IpadPro(String name) {
        this.name = name ;
        Date now = new Date(); // 시간정보를 가져오기 위해 Date 클래스 객체로 생성
        SimpleDateFormat sdf = new SimpleDateFormat("yyMMdd");
        productDate = sdf.format(now); // 연원일 정보를 얻어서 인스턴스 변수에 저장
        cnt ++;  // 생성자 : 클래스가 객체로 만들어질때 호출
        productDate += cnt ;
    }
    ...
    void setNetwork() {
        ...
        while(true){
            ...
            System.out.println("네트워크를 다시입력하세요.");
        }
    }
    void setName(){
        Scanner sc = new Scanner(System.in);
        System.out.println("각인 서비스를 신청 하시겠습니까?");
        String service = sc.next(); //각인 서비스 신청 여부를 묻는 문자열
        if(service.equalsIgnoreCase("yes")){
        System.out.print("이름을 입력하세요 : ");
        name = sc.next();}
    }
    // 시리얼넘버란 제품의 고유번호이며 제품마다 유일한 값을 가지고있다.
