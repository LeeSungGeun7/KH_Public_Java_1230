    ...
    public void run(){
        ...
        while(true) {
            try {
                ...
                special = (int)(Math.random() * 18); // 5%의 확률로 궁극기 공격
                if(normal == 1) {
                    System.out.println("물리 공격 : " + warrior.name + "에게" + wizard.pAttack() +"데미지를 입힘");
                    endGame = warrior.setDamage(wizard.pAttack());
                } else {
                    System.out.println("마법 공격 : " + warrior.name + "에게" + wizard.mAttack() +"데미지를 입힘");
                    endGame = warrior.setDamage(wizard.mAttack());
                }
                if(special == 10) {  // 5% 확률로 발동 됨
                    System.out.println(wizard.name + "궁극기 발동 "+warrior.name+"에게"+wizard.ultimate()+"데미지를 입힘");
