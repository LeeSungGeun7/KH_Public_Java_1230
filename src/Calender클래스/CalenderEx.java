...
// Calendar 클래스는 추상 클래스이므로 객체를 생성 할 수 없음 , 정적 메소드인 getInstance 메소드로 운영체제의 시간을 가져옴
// Calendar 클래스 포함된 모든 필드는 정적 변수(클래스 변수) 임 .
public class CalenderEx {
    public static void main(String[] args) {
        Calendar now = Calendar.getInstance(); // Date now = new Date();
        System.out.println(now.get(Calendar.YEAR));
        System.out.println(now.get(Calendar.MONTH) + 1); // 달의 정보는 배열의 인덱스로 넘어 오기때문에 1을 더해야함
        ...
        // yyyy년MM월dd HH시mm분ss초 형태를 Calendar를 이용해 출력해보기
